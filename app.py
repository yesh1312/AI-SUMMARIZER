...
# Accept the API key provided by the user in the prompt if not already set,
# or provide an input field for it.
...
if api_key:
    ...
    uploaded_files = st.sidebar.file_uploader("Upload PDF Papers", type="pdf", accept_multiple_files=True)
    ...
    if st.sidebar.button("Ingest Uploaded Papers"):
        if uploaded_files:
            ...
            st.sidebar.warning("Please upload some PDFs first.")
    ...
    if st.button("Query"):
        if os.path.exists("storage"):
            ...
            st.error("Vector index not found. Please ingest some papers first by uploading them in the sidebar.")
else:
